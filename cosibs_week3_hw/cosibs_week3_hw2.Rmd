...
##### A. Generate data of two random samples of size $n=30$ from a normally distributed population with mean $\mu=10$ and $\sigma = 5$. The two samples can be considered an independent pair of samples.
```{r}
set.seed(1);
...
```
This means that 5.06% of $p$-values (or 506 out of the total 10,000) are NOT greater than 0.05.
...
Our null hypothesis is that there is no evidence of a difference between the two population means. Much like in the previous homework (HW #1), the vast majority of $p$-values had a value *greater* than 0.05 (9,494 out of 10,000 to be exact). Therefore we fail to observe a statistically significant difference between the two sample means, and thus fail to reject the null hypothesis.
